...
Primarily that means appropriate handling of threads, most likely by emulating the functionality of starting and joining them.
In a Petri-Net that maps simply to a transition that produces a token on two separat places or consuming from two places respectively.
On the other the model for guarding critical sections has to be refined further.
...
The movement then indicates that the previous local cannot access the data anymore, quite similar to the rust ownership model.
If this can be done close to the rust semantics it might already fix the problem with marking mutexes.

Given a solid translation model with a reasonable control flow emulation, more complex szenarios should be testet.
This could include artificial ones like Dijkstra's dining philosophers \cite or real life programs.
This would be critical to decide if the verification process is efficient enough to be used in authentic use cases.
Such tests would also improve from more sophisticated verification results.
